...
function alert_showmsg(msgs)
{
    ...
    for (var m = 0; m < msgs.length; m++)
    {
        ...
        $( "#" + msgs[m]["input_element"].id ).addClass("inputerror");
...
function TestRut(objValue, strError)
{
	var ret = true;
	if (objValue.value.length > 0 && !validateRut(objValue.value))
    {
	
        if (!strError || strError.length == 0)
        {
            strError = objValue.name + ": Enter a valid Rut ";
        } //if                                               
        sfm_show_error_msg(strError, objValue);
        ret = false;
	}
